...
export function getRoutes  (isCluster: boolean){

	const service = new UserService(isCluster);
	//TODO refactor
	const routes: Route[] =   [
		{
		...
		},
		{
			...
			handler: async (req, res) => {
				try{
					...
					//если нет пользователя идет ошибка 
					const user = await service.getUserById(userId);
					if(!user){			
						sendError(res, StatusCode.NOT_FOUND, ErrorMessage.USER_NOT_FOUND);
						return;
					...
					if(isCluster){
						// Отправляем запрос в балансировщик 
						process.send?.({ action:UserActions.UPDATE_USER_BY_ID, payload:{id:userId, fields:JSON.parse(body)} 
							//  method: HttpMethod.PUT, endpoint:"/api/users/:userId", userId, body 
						});
						// TODO понять  Ждем ответа от балансировщика
						process.once("message", (response: Message) => {
							if (response.data) {
								sendResponse(res, StatusCode.OK, response.data);
							} else {
								sendError(res, StatusCode.SERVER_ERROR, ErrorMessage.SERVER_ERROR);
							}
						});
